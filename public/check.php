...
$strOk = '<span style="color:green;"><i class="fa fa-check"></i></span>';
$strFail = '<span style="color:red;"><i class="fa fa-times"></i></span>';
...
<body>
<div class="wrapper">
    <a href="#" title="Laravel PHP Framework">
  ...
  <div class="show_result">
    <p<?php echo $requirements['php_version'] ?  "" : ' class="error"' ; ?>>
        PHP <?php
        if (is_array($reqList[$laravelVersion]['php'])) {
            $phpVersions = array();
            foreach ($reqList[$laravelVersion]['php'] as $operator => $version) {
        ...
        echo "  ". ($requirements['php_version'] ? $strOk : $strFail."<br /> Your current php version is : <b>".phpversion()."</b> need to be minimum 7.1.3 <br />"); ?>
    </p>


    <?php if ($reqList[$laravelVersion]['openssl']) : ?>
        <p<?php echo $requirements['openssl_enabled'] ?  "" : ' class="error"' ; ?>>
        OpenSSL PHP Extension <?php echo $requirements['openssl_enabled'] ? $strOk : $strFail; ?></p>
    <?php endif; ?>

    <?php if ($reqList[$laravelVersion]['pdo']) : ?>
        <p<?php echo $requirements['pdo_enabled'] ?  "" : 'class="error"' ; ?>>
        PDO PHP Extension <?php echo $requirements['pdo_enabled'] ? $strOk : $strFail; ?></p>
    <?php endif ?>

    <?php if ($reqList[$laravelVersion]['mbstring']) : ?>
        <p<?php echo $requirements['mbstring_enabled'] ?  "" : ' class="error"' ; ?>>
        Mbstring PHP Extension <?php echo $requirements['mbstring_enabled'] ? $strOk : $strFail; ?></p>
    <?php endif ?>

    <?php if ($reqList[$laravelVersion]['tokenizer']) : ?>
        <p<?php echo $requirements['tokenizer_enabled'] ?  "" : ' class="error"' ; ?>>
        Tokenizer PHP Extension <?php echo $requirements['tokenizer_enabled'] ? $strOk : $strFail; ?></p>
    <?php endif ?>


    <?php if ($reqList[$laravelVersion]['xml']) : ?>
        <p<?php echo $requirements['xml_enabled'] ?  "" : ' class="error"' ; ?>>
        XML PHP Extension <?php echo $requirements['xml_enabled'] ? $strOk : $strFail; ?></p>
    <?php endif ?>

    <?php if ($reqList[$laravelVersion]['ctype']) : ?>
        <p<?php echo $requirements['ctype_enabled'] ?  "" : ' class="error"' ; ?>>
         CTYPE PHP Extension <?php echo $requirements['ctype_enabled'] ? $strOk : $strFail; ?></p>
    <?php endif ?>

    <?php if ($reqList[$laravelVersion]['json']) : ?>
        <p<?php echo $requirements['json_enabled'] ?  "" : ' class="error"' ; ?>>
        JSON PHP Extension <?php echo $requirements['json_enabled'] ? $strOk : $strFail; ?></p>
    <?php endif ?>

    <?php if ($reqList[$laravelVersion]['mcrypt']) : ?>
        <p<?php echo $requirements['mcrypt_enabled'] ?  "" : ' class="error"' ; ?>>
        Mcrypt PHP Extension <?php echo $requirements['mcrypt_enabled'] ? $strOk : $strFail; ?></p>
    <?php endif ?>

    <?php if (!empty($reqList[$laravelVersion]['obs'])): ?>
        <p><?php echo $reqList[$laravelVersion]['obs'] ?></p>
    <?php endif; ?>
  </div>
</div>
</body>
