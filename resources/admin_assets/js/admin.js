$( function() {  
 ...
 function addNewCategory(catType, url='', productID='' ){
    var title = $("#catType_"+catType + " .catName").val();
    var parent = $("#catType_"+catType + " .catParent").val();
    var readyData = "title="+title+"&productID="+productID+"&parent="+parent+"&type="+catType ;
...
function access_url(url, resultShow, load_class="none", get_data="none", readyData="") {
 ...
 var notice_class= load_class =="none"? resultShow : load_class;
      $(notice_class).html('<img style="margin:0 auto;" src="'+url_resources+'admin_assets/img/ajax-loader.gif" width="20" height="20" />');  
...
function updateSerialiseFromDiv(url, resultClass, divSerialise="none"  ) {
        ...
        $(resultClass).html('<img style="margin:0 auto;" src="'+url_resources+'admin_assets/img/ajax-loader.gif" width="20" height="20" />');  
...
function save_field(url, notice_class, id, field, class_input, confirm_message = "") {
    var value_input = $(class_input).val();
    var current_value = $(class_input).data("value");
   
    if (current_value == value_input){
         return false;
        }
...
function upload_files(url_upload, id_inp, cl_res, class_show_load, parent_id, field_1='' ,field_2=''  ){
...
function edit_row_tr(id){
   ...
   $(id + " input, "+ id + " select, "+ id + " textarea, "+ id + " label").addClass('show_edit');
}
